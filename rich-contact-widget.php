...
class RC_Widget extends WP_Widget {
    ...
    public function widget( $args, $instance ) {
        ...
        $widget_output = '<ul class="h-card">';
                ...
                if ( !empty( $instance['postal_code'] ) || ( get_locale() == 'en_US' && !empty( $instance['state'] ) ) ||!empty( $instance['city'] ) ) {
                    if ( get_locale() == 'en_US' ) {
                        if ( !empty( $instance['city'] ) ) {
                            $widget_output .= '<span class="p-locality">' . $instance['city'] . '</span>&nbsp;';
                        }
                        if ( !empty( $instance['state'] ) ) {
                            $widget_output .= '<span class="p-region">' . $instance['state'] . '</span>&nbsp;';
                        }
                        if ( !empty( $instance['postal_code'] ) ) {
                            $widget_output .= '<span class="p-postal-code">' . $instance['postal_code'] . '</span><br />';
                        }
                    }
                    else {
                       if ( !empty( $instance['postal_code'] ) ) {
                           $widget_output .= '<span class="p-postal-code">' . $instance['postal_code'] . '</span>&nbsp;';
                       }
                       if ( !empty( $instance['city'] ) ) {
                           $widget_output .= '<span class="p-locality">' . $instance['city'] . '</span><br />';
                       }
                    }
                }
            ...
            if ( !empty( $instance['fax'] ) ) {
                $widget_output .= '<li class="p-tel-fax">';
    ...
    public function form( $instance ) {
       ...
       $widget_form_output .= $this->types_options( $types, -1, $instance['type'] );
        ...
        $widget_form_output .= '<p>
            <label for="' . $this->get_field_id( 'fax' ) . '">' . __( 'Fax :', 'rich-contact-widget' ) . '</label>
        ...
        $widget_form_output .= '<p>
            <label for="' . $this->get_field_id( 'web' ) . '">' . __( 'Web address :', 'rich-contact-widget' ) . '</label>
